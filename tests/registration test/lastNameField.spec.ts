...
import { ERRORS_LAST_NAME, ERROR_COLOR } from '../../test-data/errors';
...
test.describe('Last name field', () => {
    ...
    test('shouldn\'t see an error message when filling in the last name field with correct name', async ({page}) => {
        ...
        await expect(signUpForm.lastNameField).toHaveValue(REGISTRATION_VALID_USER_DATA.lastName);
    ...
    test('should see an error message when filling in the last name field with empty name', async ({page}) => {
        ...
        await expect(errorMessageLocator).toHaveText(ERRORS_LAST_NAME.empty);
    });
    ...
    INVALID_NAMES.forEach(name => {
        test(`should see an error message when filling in the last name field with incorrect name: ${name}`, async ({page}) => {
            ...
            await signUpForm.lastNameField.fill(name);
            await page.keyboard.press('Tab');
            let errorMessageLocator = signUpForm.error;

            if (name.length < 2) {
                await expect(errorMessageLocator).toHaveCSS('color', ERROR_COLOR);
                await expect(errorMessageLocator).toHaveText(ERRORS_LAST_NAME.wrongLength);
            } else if (name.length > 20) {
                await expect(errorMessageLocator).toHaveCSS('color', ERROR_COLOR);
                await expect(errorMessageLocator).toHaveText(ERRORS_LAST_NAME.wrongLength);
            } else {
                await expect(errorMessageLocator).toHaveCSS('color', ERROR_COLOR);
                await expect(errorMessageLocator).toHaveText(ERRORS_LAST_NAME.wrongData);
            }
        });
    });
    ...
    test('should see an erros if we fill last name with space', async ({page}) => {
        ...
        await expect(signUpForm.error).toHaveText(ERRORS_LAST_NAME.wrongData + ERRORS_LAST_NAME.wrongLength);
    });
})